...
import testPixelData from './testData'
import Points from './scratchPad'

const accessToken = 'pk.eyJ1Ijoic2N2NG1lIiwiYSI6ImNpdGRtamM0YjAwdWQyem15d2h1NXM5dzAifQ.3et01XTEJVX57sWa54WDEA';
...
class MapExample extends Component {
  ...
  componentWillMount() {
    ...
    this.annotations = [];
    Points.genColors(testPixelData);
    this.prepLatLngData();
  }
  ...
  prepLatLngData() {
    let { pixels } = testPixelData;
    pixels.forEach((pixel, index) => {

      this.annotations.push({
        coordinates: Points.calcPolyPoints(pixel),
        type: 'polygon',
        strokeAlpha: 1,
        fillAlpha: 1,
        fillColor: Points.calcColorValue(pixel),
        id: `foobar${index}`
      })
  ...
  transition = () => {
    this.annotations.forEach((a) => {
      a.fillColor = Points.blueColorValue(a)
    })
  };
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <View>
          <Text onpress={this.transition}>Switch</Text>
        </View>
        <MapView
...
AppRegistry.registerComponent('MapboxTest', () => MapExample);
